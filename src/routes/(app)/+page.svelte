<script lang="ts">
  ...
  $: if ($dates.errored !== true) {
    displayDates = Object.values(
      $dates.reduce(
        (accumulator, date) => {
          const itemKey =
            date?.group != undefined && date.group != ""
              ? `GROUP-${date?.group}`
              : `ID-${date.id}`;
          if (accumulator[itemKey] === undefined) {
            ...
            compareAsc(
              parseISO(date.date),
              parseISO(accumulator[itemKey].date),
            ) < 0
          ) {
            accumulator[itemKey] = date;
          }
      ...
      return (
        parseISO(event1EffectiveDate).getTime() -
        parseISO(event2EffectiveDate).getTime()
      );
    });
  }
  ...
  function updateTime() {
    ...
    if (nextAttemptMarker !== undefined) {
      timeToNextAttempt = formatDistanceStrict(nextAttemptMarker, now, {
        unit: "second",
        roundingMethod: "ceil",
      });
    }
  ...
  async function updateDates() {
    ...
    if (result === -1) {
      ...
      dateUpdateTimer = setTimeout(
        updateDates,
        differenceInMilliseconds(result, new Date()),
      );
    } else {
...
<svelte:head>
  <title>Home | OWCountdown.com</title>

  <meta
    name="og:title"
    content="OWCountdown.com | Live countdowns to important Overwatch events"
  />
  <meta
    name="og:description"
    content="Count down the time left until important Overwatch events while seeing the times in your timezone! Visit an event page to get more information about the event, or copy the date and time down for later use."
  />

  <meta name="og:url" content="https://owcountdown.com" />
  <meta name="og:image" content="https://owcountdown.com/og-image.jpg" />
  <meta name="og:image:alt" content="OWCountdown Logo" />
...
{#if $dates.errored}
  <div
    class="fixed top-0 left-0 z-30 w-full text-center p-4 bg-[#f15047df] dark:bg-[#7F1D1DDF] dark:text-zinc-50"
  >
    A problem arose while contacting the server for updated information. Don't
    refresh, we'll try to contact the server again for you {nextAttemptMarker !==
      undefined && compareAsc(now, nextAttemptMarker) < 0
...
<div class="relative min-h-full items-center py-2 w-full dark:text-zinc-50">
  ...
  {:else if loading}
    <div
      class="absolute top-[42vh] left-1/2 -translate-x-1/2 -translate-y-1/2 text-center"
      transition:fade
    >
  ...
  {:else}
    <div
      class="absolute top-[42vh] left-1/2 -translate-x-1/2 -translate-y-1/2 w-max text-center"
      transition:fade
    >
      <h1 class="text-5xl text-ow2-orange dark:text-ow2-light-orange">
        No events found
  ...
  <div class="absolute w-full top-0 bottom-0 pointer-events-none">
    <div class="relative grid grid-rows-[1fr_auto] h-full w-full">
      <div></div>
      <div
        class={`sticky bottom-8 flex flex-row-reverse pr-4 sm:pr-8 z-20 pointer-events-none`}
        transition:fade
      >
        {#if $page.data.user && !loading}
          <a
            href="/event/new"
          ...
          >
            <FontAwesomeIcon icon={faPlus} /><span class="pl-2 font-semibold"
              >New Event</span
            >
          </a>
        {/if}
