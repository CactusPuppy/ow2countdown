<script lang="ts">
  ...
  import { addSeconds, compareAsc, format, formatISO, getMilliseconds, parseISO } from "date-fns";
...
<div class="flex flex-col justify-center items-center h-[100vh] w-[100vw]">
  <div class="flex-grow flex flex-col justify-center items-center py-8 w-[100vw] dark:text-zinc-50">
    ...
    <p class="text-center text-xl md:text-2xl lg:text-3xl text-ow2-orange dark:text-ow2-light-orange">
      {activeDate ? activeDate.title : "Loading..."}
    </p>
    <p class="text-center text-md md:text-xl lg:text-2xl">
      {activeDate ? format(parseISO(activeDate?.date), "PPPPp") : ""}
    </p>
    <div class="mt-6">
